...
// --- metadata dialog (moved to src/ui/metadata-dialog.js — M3) ---------------------
// 文件/文档/统计三区的收集与渲染都在模块内；此处注入读取本文件状态的 getter，
// 使模块不反向依赖 main.js 的模块级可变变量。
const metadataDialog = createMetadataDialog({
  ...
  getLastSavedXml: () => lastSavedXML,
  getPlatformLabel: () => (PLATFORMS[currentPlatform] && PLATFORMS[currentPlatform].label) || '—',
...
$('#btn-info').addEventListener('click', metadataDialog.open);
...
// metadata modal
$('#btn-info-close').addEventListener('click', metadataDialog.hide);
$('#btn-info-done').addEventListener('click', metadataDialog.hide);
$('#btn-info-refresh').addEventListener('click', metadataDialog.open);

// close overlays with Escape
document.addEventListener('keydown', (e) => {
  if (e.key !== 'Escape') return;
  if (!els.errorOverlay.classList.contains('hidden')) hideError();
  else if (!els.infoModal.classList.contains('hidden')) metadataDialog.hide();
...
if (studio) {
  studio.onMenu(async (action) => {
    switch (action) {
      ...
      case 'file-info': return metadataDialog.open();
